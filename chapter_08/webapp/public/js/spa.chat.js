...
spa.chat = (function () {
    ...
    var
        ...
        setSliderPosition, onTapToggle, onSubmitMsg, onTapList, onSetchatee, onUpdatechat, 
        onListchange, onLogin, onLogout, configModule, initModule, removeSlider, handleResize;
     ...
     *      - callback - optional callback to be run at the end of slider animation.
     ...
     *      - If the requested position is the current poistion, it returns true without taking further action.
     * returns: boolean
    ...
    setSliderPosition = function (position_type, callback) {
        ...
        // position type of 'opened' is not allowed for anon user; therefore we simply return false
        if (position_type === 'opened' && configMap.people_model.get_user().get_is_anon()) {
            return false;
        }

        // returns true if slider already in requested position
     ...
     * params:
     *      - $append_target - a jQuery collection that should represent a single DOM container
     * actions:
     *      - appends the chat slider to the provided conatiner and fills it with HTML content.
     *      - Then initializes elements, events, and handlers to provide the user with a chat-room interface
    ...
    removeSlider = function () {
        // unwind inintialization and state
     ...
     * actions:
     *      - if the window height of width falls below a given threshold, resize the chat slider for the reduced window size.
     * returns: boolean
    ...
    handleResize = function () {
        // don't do anythin if we don't have a slider container
        if (!jqueryMap.$slider) {
            return false;
        }
